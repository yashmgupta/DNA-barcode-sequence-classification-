...
class TNLayer(tf.keras.layers.Layer):
    def __init__(self, units=3,in_dim=28, out_dim=10,bond_dim=10):
        ...
        self.tensor_list = [tf.Variable(tf.random.uniform(shape =(in_dim,bond_dim)),trainable=True)]+ [tf.Variable(tf.random.uniform(shape =(in_dim, bond_dim, bond_dim)),trainable=True) for i in range(1, units//2)] + [tf.Variable(tf.random.uniform(shape =(in_dim,bond_dim,bond_dim,out_dim)),trainable=True)]+[tf.Variable(tf.random.uniform(shape =(in_dim, bond_dim, bond_dim)),trainable=True) for i in range(units//2+1, units-1)]+[tf.Variable(tf.random.uniform(shape =(in_dim,bond_dim)),trainable=True)]
    ...
    def call(self, inputs):
        def TN_contraction(input_i, tensor_list, bias, units, in_dim, out_dim, bond_dim):
            ...
            for t_i in range(1,units-1):
                mpo[t_i][2]^mpo[t_i+1][1]
